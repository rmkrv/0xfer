...
import com.android.qttransfer.transfer.FileChunker
import com.android.qttransfer.transfer.FileInfo
import com.android.qttransfer.transfer.FountainDecoder
import com.android.qttransfer.transfer.FountainEncoder
import com.android.qttransfer.transfer.PacketCodec
import com.android.qttransfer.qr.TrackedQrDecoder
...
class SmokeTest {
    ...
    fun decimenV3HeaderMatchesPublishedWireVector() {
        val actual = PacketCodec.encode(
            com.android.qttransfer.transfer.FountainPacket(
                sessionId = 0xBEEF,
                sequence = 0x01020304,
    ...
    fun decimenCarouselRoundTripWithoutMetadata() {
        ...
        val blockLength = 120
        val container = com.android.qttransfer.transfer.DecimenContainer.pack("roundtrip.bin", "application/octet-stream", data)
        val enc = FountainEncoder(container, blockLength, 123)
        val first = PacketCodec.decode(PacketCodec.encode(enc.nextPacket()))
        val rcv = com.android.qttransfer.transfer.TransferReceiver(first)
        rcv.receive(first)
        var frames = 1
    ...
    fun decimenCarouselSurvivesOutOfOrderFrames() {
        ...
        val blockLength = 240
        val container = com.android.qttransfer.transfer.DecimenContainer.pack("parallel.bin", "application/octet-stream", data)
        ...
        frames.reverse()
        val receiver = com.android.qttransfer.transfer.TransferReceiver(frames.first())
        for (frame in frames) {
